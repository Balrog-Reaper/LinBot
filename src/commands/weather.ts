...
export const weather: BotCommand = {
    name: "weather",
    description: "`>weather [國家] [城市(選填)]` 查詢天氣狀況。例如：`>weather 台灣` 或 `>weather 台灣 高雄`。",
    category: "public",
    dmAllowed: false,
    ownerOnly: false,
    ...
    async execute(msg: Message, args: string[]): Promise<void> {
        ...
        try {
            ...
            if (weatherData.isDetailed) {
                // ─── 詳細模式：單一 Embed，不需要翻頁 ───
                const embed = formatWeatherDetailEmbed(weatherData.weatherList[0] as WeatherDetailItem);
                ...
                collector.on("end", async () => {
                    await replyMsg.edit({ components: [] })
                        .catch((error: unknown) => {
                            ...
                            console.error("移除按鈕失敗：", errMsg);
                        });
                });
            }
        } catch (error: unknown) {
            const errMsg = error instanceof Error ? error.message : String(error);
            console.error("❌ 天氣查詢錯誤：", errMsg);
