...
export const userInfo: BotCommand = {
    name: "userInfo",
    description: "`>userInfo @某人` 查詢指定成員的基本資訊。",
    category: "moderator",
    dmAllowed: false,
    ownerOnly: false,
    ...
    async execute(msg: Message): Promise<void> {
        // 1. 取得指令中被標註的對手 (第一位)
        const targetMember: GuildMember | undefined = msg.mentions.members?.first();

        // 如果沒有標註對象，提醒主人
        if (!targetMember) {
            await msg.reply("主人，請標註一位您想調查的對象喔！例如：`>userInfo @某人` 🦊");
            return;
        }
        ...
        try {
            ...
            console.error("無法發送私訊給使用者:", errMsg);
        }
    }
};
