...
const providers: Record<string, WeatherProvider> = {
    taiwan: {
        ...
        resolveCity: (input: string): string | null => {               // 城市名稱解析器
...
/**
 * 天氣查詢主入口
 * @param country - 國家名稱（必填）
 * @param city - 城市名稱（選填）
 * @returns 天氣資料物件
 */
export async function queryWeather(country: string, city: string | null): Promise<WeatherQueryResult> {
