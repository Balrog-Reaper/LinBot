...
import type { ParsedTimeResult } from "./schedulerTypes.js";

// 時間解析專用的 System Prompt
const TIME_PARSER_PROMPT: string = `你是一個精確的時間解析器，你的唯一任務是將使用者的口語化時間描述轉換為標準時間格式。
...
/**
 * 取得指定時區的當前格式化時間
 * @returns 例如 "2026-04-06T22:30:00+08:00 (星期一)"
 */
function getCurrentFormattedTime(): string {
    ...
    */
    ...
    const getVal = (type: Intl.DateTimeFormatPartTypes): string => parts.find(p => p.type === type)!.value;
 ...
 *
 * @param rawInput - 使用者原始輸入（例如「明天早上八點 記得繳報告」）
 * @returns 解析結果，或 null 表示解析失敗
 */
export async function parseTimeWithLLM(rawInput: string): Promise<ParsedTimeResult | null> {
    ...
    try {
        ...
        const parsedData = JSON.parse(text) as ParsedTimeResult;
