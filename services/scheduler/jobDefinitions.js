            ...
            if (error.code === 50007) {
                ...
                if (dmRetryCount >= DM_BLOCKED_MAX_RETRIES) {
                    ...
                    try {
                        ...
                        if (channel) {
                            await channel.send(buildDmGiveUpNotice(userId, content, dmRetryCount));
                        }
                    } catch (notifyError) {
                        console.error(`❌ 發送放棄通知失敗：`, notifyError.message);
                    }
                ...
                try {
                    const channel = await client.channels.fetch(channelId);
                    if (channel) {
                        await channel.send(buildDmBlockedNotice(userId, content, dmRetryCount + 1, DM_BLOCKED_MAX_RETRIES));
                    }
                } catch (channelError) {
                    console.error(`❌ 發送頻道降級訊息失敗：`, channelError.message);
                }
